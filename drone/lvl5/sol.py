...
def solve(f):
    drones, parcels = [int(el) for el in f.readline().split()]
    capacities = [float(f.readline()) for _ in range(drones)]
    print(drones,parcels)
    print(capacities)
    parcels = [[float(el) for el in f.readline().split()] for _ in range(parcels)]
    print(drones,parcels)
    drones = [Drone(i,pos) for i,pos in enumerate(parcels) if i<drones]
